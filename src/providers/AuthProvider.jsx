...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [myPropertyRes, allPropertyRes, reviewsRes, agentRes] =
          await Promise.all([
        ...
        setDashboardData({
          myProperty: myPropertyData?.length ?? 0,
          allProperty: allPropertyData?.length ?? 0,
          verifiedProperties:
            allPropertyData?.filter((p) => p?.isVerified)?.length ?? 0,
